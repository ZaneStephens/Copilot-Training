...
// - As you type (2+ chars) a dropdown of live suggestions appears, ranked by relevance
// - Up/Down arrow keys move between suggestions; Enter opens the highlighted one (or runs full-page search)
// - When you navigate to a result via a suggestion or full search, matching terms are highlighted
//   inside the loaded document and the page scrolls to the first hit
// - Esc closes the suggestion dropdown
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ctrl/Cmd + K focuses search globally
    document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Escape' && suggestionsBox && suggestionsBox.classList.contains('open')) {
            closeSuggestions();
        }
    });
    ...
    searchInput.addEventListener('keydown', function(e) {
        if (!suggestionsBox || !suggestionsBox.classList.contains('open')) {
        ...
        if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            if (focusedSuggestionIdx >= 0 && currentSuggestions[focusedSuggestionIdx]) {
                navigateToSuggestion(currentSuggestions[focusedSuggestionIdx]);
            } else {
                runFullSearch();
            }
        }
    ...
    searchInput.addEventListener('input', function() {
        ...
        typingTimer = setTimeout(() => updateSuggestions(q), 180);
    });
    ...
    function moveSuggestionFocus(delta) {
        ...
        focusedSuggestionIdx = (focusedSuggestionIdx + delta + currentSuggestions.length) % currentSuggestions.length;
        suggestionsBox.querySelectorAll('.search-suggestion').forEach((el, idx) => {
            el.classList.toggle('focused', idx === focusedSuggestionIdx);
        });
    }

    async function updateSuggestions(query) {
        const matches = await getQuickMatches(query, 6);
        currentSuggestions = matches;
        focusedSuggestionIdx = -1;
    ...
    function renderSuggestions(matches, query) {
        ...
        if (!matches.length) {
            suggestionsBox.innerHTML = `
                <div class="search-suggestions-empty">No matches. Press Enter to search the whole archive.</div>
                <div class="search-suggestions-footer">
                    <span>${escapeHtml(query)}</span>
                    <span><span class="search-shortcut-hint">Enter</span> to search all</span>
                </div>
            `;
        ...
        const items = matches.map((m, idx) => `
            <div class="search-suggestion${idx === focusedSuggestionIdx ? ' focused' : ''}"
                 data-idx="${idx}" role="option">
                <div class="search-suggestion-icon"><i class="${m.icon}"></i></div>
                <div class="search-suggestion-body">
                    <div class="search-suggestion-title">${escapeHtml(m.title)}</div>
                    <div class="search-suggestion-snippet">${m.snippetHtml}</div>
                </div>
            </div>
        `).join('');
        ...
        suggestionsBox.innerHTML = `
            ...
            <div class="search-suggestions-footer">
                ...
                <span><span class="search-shortcut-hint">Enter</span> to open · <span class="search-shortcut-hint">↑↓</span> to navigate</span>
            </div>
        `;
        ...
        suggestionsBox.querySelectorAll('.search-suggestion').forEach(el => {
            el.addEventListener('click', () => {
                ...
                if (currentSuggestions[idx]) navigateToSuggestion(currentSuggestions[idx]);
            });
            el.addEventListener('mouseenter', () => {
                focusedSuggestionIdx = parseInt(el.dataset.idx, 10);
                suggestionsBox.querySelectorAll('.search-suggestion').forEach((s, i) => {
                    s.classList.toggle('focused', i === focusedSuggestionIdx);
                });
            });
        });
    ...
    function navigateToSuggestion(suggestion) {
        ...
        closeSuggestions();
        // Stash terms so the content loader can highlight them after the doc renders
        window.__pendingHighlight = query;
        loadContent(suggestion.contentId);
        window.history.pushState({}, '', `#${suggestion.contentId}`);
        const navLinks = document.querySelectorAll('.main-nav a, .dropdown-content a');
        navLinks.forEach(l => l.classList.remove('active'));
        const matchingLink = document.querySelector(`.main-nav a[href="#${suggestion.contentId}"]`) ||
                           document.querySelector(`.dropdown-content a[href="#${suggestion.contentId}"]`);
        if (matchingLink) matchingLink.classList.add('active');

        // After the doc loads + renders, highlight the term and scroll to the first hit
        setTimeout(() => highlightInPage(query), 350);
    }
});

// === Quick suggestion engine (in-memory cached scan over all known docs) ===

const _docCache = new Map(); // path -> raw markdown

async function getDocText(path) {
    if (_docCache.has(path)) return _docCache.get(path);
    try {
        const r = await fetch(path);
        if (!r.ok) return '';
        const t = await r.text();
        _docCache.set(path, t);
        return t;
    } catch (e) {
        return '';
    }
...
async function getQuickMatches(query, limit) {
    const q = query.toLowerCase();
    const terms = q.split(/\s+/).filter(t => t.length > 1);
    if (!terms.length) return [];

    const allEntries = [
        ...Object.entries(contentMap).filter(([id, p]) => typeof p === 'string' && !p.startsWith('custom-')),
        ...Object.entries(promptExamplesMap)
    ];

    const results = [];
    for (const [id, path] of allEntries) {
        const content = await getDocText(path);
        if (!content) continue;
        ...
        let score = 0;
        // Exact phrase
        const phraseHits = (lower.match(new RegExp(escapeRegex(q), 'g')) || []).length;
        score += phraseHits * 5;
        // Title hits
        ...
        // Header hits
        const headerHits = (content.match(new RegExp(`^#{1,3} .*(${terms.map(escapeRegex).join('|')}).*$`, 'gim')) || []).length;
        score += headerHits * 3;

        if (score === 0) continue;

        const snippetHtml = buildSnippet(content, terms, q);
        const isPrompt = path.includes('Prompt examples');
...
function buildSnippet(content, terms, fullQuery) {
    const lower = content.toLowerCase();
    ...
    let raw = content.slice(start, end).replace(/\s+/g, ' ').trim();
    if (start > 0) raw = '…' + raw;
    if (end < content.length) raw = raw + '…';

    let html = escapeHtml(raw);
    [fullQuery, ...terms].forEach(term => {
        if (term.length < 2) return;
        html = html.replace(new RegExp(escapeRegex(term), 'gi'), (m) => `<mark>${m}</mark>`);
    });
    return html;
}

function iconForId(id, isPrompt) {
    const map = {
    ...
    if (isPrompt) return 'fas fa-lightbulb';
...
// === In-page highlight ===
//
// After navigating to a doc from a search result, walk the rendered text nodes
// and wrap every match in <mark class="in-page-hit">. Then scroll to the first hit.
...
function highlightInPage(query) {
    ...
    active.querySelectorAll('mark.in-page-hit').forEach(m => {
    ...
    const walker = document.createTreeWalker(active, NodeFilter.SHOW_TEXT, {
        acceptNode: (node) => {
            ...
            // Skip script/style and existing marks
            const parent = node.parentElement;
            if (!parent) return NodeFilter.FILTER_REJECT;
            const tag = parent.tagName;
            if (tag === 'SCRIPT' || tag === 'STYLE' || tag === 'MARK') return NodeFilter.FILTER_REJECT;
            // Skip the sticky sidebar
            if (parent.closest('.doc-sidebar')) return NodeFilter.FILTER_REJECT;
    ...
    // Auto-clear highlights after 30s so they don't linger forever
    clearTimeout(window.__highlightClearTimer);
    window.__highlightClearTimer = setTimeout(() => {
        active.querySelectorAll('mark.in-page-hit').forEach(m => {
            const parent = m.parentNode;
            if (!parent) return;
            parent.replaceChild(document.createTextNode(m.textContent), m);
