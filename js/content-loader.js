...
const contentMap = {
    ...
    'frontier': 'Documents/Frontier.md',
    ...
    const html = `
        ...
        <section class="guides-container">
            <div class="card-grid">
                ...
                <div class="card" data-target="frontier">
                    <div class="card-icon icon-purple"><i class="fas fa-rocket"></i></div>
                    <h4>Frontier &amp; CoWork</h4>
                    <p>Early-access program covering Researcher, Analyst, Notebooks, Tuning, and Copilot CoWork</p>
                </div>
            </div>
        </section>
    `;
...
function applyContentEnhancements(contentElement) {
    ...
    // Build sticky sidebar TOC (skip on landing/home/error containers)
    buildStickySidebarToc(contentElement);
    ...
    links.forEach(link => {
        ...
        // Internal document links without hash/fragment
            ...
            if (contentMap[targetId] || targetId === 'home' || targetId === 'guides' ||
                targetId === 'apps' || targetId === 'prompt-examples' || targetId === 'frontier') {
...
// Build a sticky sidebar table of contents alongside the document content.
// Wraps the rendered markdown in a two-column grid: [main content] [sticky outline].
// Uses IntersectionObserver to highlight the currently visible heading.
function buildStickySidebarToc(contentElement) {
    if (!contentElement || contentElement.dataset.sidebarBuilt === 'true') return;

    const headings = Array.from(contentElement.querySelectorAll('h2, h3'))
        .filter(h => {
            const id = (h.id || '').toLowerCase();
            return id !== 'table-of-contents' && id !== 'contents';
        });

    if (headings.length < 3) return; // Not enough structure to bother

    // Move all current children of contentElement into a new "main" wrapper
    const main = document.createElement('div');
    main.className = 'doc-main';
    while (contentElement.firstChild) {
        main.appendChild(contentElement.firstChild);
    }
    ...
    // Wrap with the grid container
    const wrapper = document.createElement('div');
    wrapper.className = 'doc-with-sidebar';
    wrapper.appendChild(main);
    wrapper.appendChild(sidebar);
    contentElement.appendChild(wrapper);
    contentElement.classList.add('has-sidebar-toc');
    contentElement.dataset.sidebarBuilt = 'true';

    // Scrollspy: highlight the sidebar entry whose heading is currently visible
    ...
    const setActive = (id) => {
        ...
        if (link) {
            ...
            // Keep the active link in view inside the sidebar
            const sbRect = sidebar.getBoundingClientRect();
            const lnkRect = link.getBoundingClientRect();
            if (lnkRect.top < sbRect.top || lnkRect.bottom > sbRect.bottom) {
    ...
    if ('IntersectionObserver' in window) {
        ...
        const observer = new IntersectionObserver((entries) => {
            ...
            // Pick the first heading (in document order) that's currently in the viewport
            const firstVisible = headings.find(h => visible.has(h.id));
            if (firstVisible) setActive(firstVisible.id);
        }, {
            // Activate when heading is roughly in the top third of the viewport
