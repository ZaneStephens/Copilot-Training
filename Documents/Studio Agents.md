...
### Multi-Agent Orchestration and Computer Use

Two newer capabilities that change what "build an agent" means:

- **Multi-agent orchestration.** An agent can call other agents as tools — including Microsoft's reasoning agents (Researcher and Analyst) where licensing permits. This is the building block for Copilot CoWork workspaces where several specialised agents collaborate with humans on a shared goal. Practical pattern: a "front-of-house" agent that delegates research to Researcher, analysis to Analyst, and policy checks to a tenant-built reviewer agent.
- **Computer-use actions.** An agent can drive a browser session or virtual desktop on the user's behalf — clicking, typing, navigating multi-step workflows in systems with no first-class API. This is the highest-risk capability in Copilot Studio: enabling it requires admin opt-in plus per-site allowlisting, and every action is logged. Use it for genuinely API-less workflows; don't use it as an API substitute for systems that do have an API.

For tenants on the Frontier program, these capabilities arrive earlier and with broader scope. See the [Frontier &amp; CoWork](#frontier) guide.
