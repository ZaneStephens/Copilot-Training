...
The Copilot admin page lives inside the Microsoft 365 admin center under the **Copilot** section. The current layout — refreshed in early 2025 and iteratively improved since — uses a tabbed structure that brings configuration, health, adoption, and the Frontier early-access program together in one place.

### Tab Breakdown

The admin page features a tabbed structure with sections for Overview, Health, Discover, Settings, and (for tenants opted in) Frontier. Each tab serves a distinct purpose in helping administrators manage and optimize Microsoft 365 Copilot.
...
A significant enhancement to the administrative experience is the ability for global administrators to grant access to the Microsoft Copilot Dashboard via Microsoft Entra ID (formerly Azure AD) groups. This pattern — group-based access rather than per-user assignment — is now the recommended default for Copilot Dashboard access, Frontier program targeting, and most other Copilot-related access decisions.
...
## Recent Admin-Side Changes Worth Knowing

The Copilot admin surface has matured significantly over 2025 and into 2026. The changes most worth flagging:

**Frontier program controls.** Tenants can now opt in to early-access Copilot features (Researcher, Analyst, Notebooks, Copilot Tuning, and Copilot CoWork) from a dedicated **Copilot → Settings → Frontier** page, with Entra-group-scoped targeting so you can pilot new capabilities to a defined population without flipping the whole tenant. See the [Frontier &amp; CoWork](#frontier) guide for the full rollout pattern.

**Agent lifecycle in the admin center.** A dedicated section under the Copilot tab now provides full lifecycle management for Copilot agents — IT-published, third-party from the agent store, and SharePoint-attached. Admins can approve, block, scope to groups, and audit consumption for each.

**Copilot agent consumption meter.** Agent runs that consume metered capacity (notably Researcher and Analyst at scale, plus pay-as-you-go Copilot Studio agents) are tracked through the consumption meter, with the option to set per-group quotas and alerts.

**Computer-use agents** require explicit per-user enablement and per-site allowlisting from the admin center before they can run. Treat this as a security control rather than a convenience setting.

**SharePoint agent governance.** Member control over approved agents has improved, including more granular permission settings and the ability to chat one-on-one with a SharePoint agent in Teams. This addresses the most-cited governance limitation from the original SharePoint agent rollout.

**Outlook integration.** Inbox prioritisation, selective text rewriting, attachment summarisation across PDF/Word/PowerPoint, and calendar meeting highlighting all reached general availability through 2025 and are now standard parts of the Copilot in Outlook experience admins can enable for their users.

### Staying Informed

The official references admins should bookmark:

- **Microsoft 365 Roadmap** ([microsoft365.com/roadmap](https://www.microsoft.com/microsoft-365/roadmap?filters=Copilot)) — preview vs. GA status across Copilot.
- **Microsoft 365 admin center → Copilot → Settings → Frontier** — the in-product source of truth for what's currently in early access for your tenant.
- **Copilot release notes** on Microsoft Learn — feature-level changelog.
- **Microsoft 365 Message Center** — tenant-specific announcements and rollout windows.
