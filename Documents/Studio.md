...
Copilot Studio has changed significantly through 2025 and into 2026. The shifts most worth knowing about as a maker:

- **Generative orchestration is the default.** Rather than hand-authoring every topic, you describe what the agent should do and Copilot Studio chooses topics, knowledge sources, and actions at runtime. Topic-based authoring is still available for cases where you need deterministic flow.
- **Multi-agent orchestration.** Agents can call other agents as tools, including Microsoft's reasoning agents (Researcher and Analyst) where the licensing permits. This is the foundation for Copilot CoWork-style workspaces where multiple specialised agents collaborate on a shared goal.
- **Computer-use actions.** Agents can drive browsers and virtual desktops to operate systems that don't have first-class APIs. This is gated behind explicit per-tenant enablement and per-site allowlisting — treat it as a security control, not a default capability.
- **Autonomous agents** are first-class. Trigger on events (file added, message received, schedule), authenticate with non-interactive credentials, and run unattended. Crucial for back-office automation.
- **Copilot Tuning (Frontier).** Low-code fine-tuning on tenant content, so an agent can match your house style and terminology without leaving the Microsoft 365 boundary.
- **Microsoft 365 Copilot Chat publishing.** Custom agents publish directly into the Copilot Chat agent sidebar and respond to `@`-mentions across Word, Excel, PowerPoint, and Chat.
- **Expanded Graph connectors and Azure OpenAI grounding** for organisations that want their agents to reach beyond Microsoft 365 into LOB systems.

For tenants on the Frontier early-access program, several of these (notably Tuning and the more powerful multi-agent and computer-use scenarios) arrive earlier than general availability — see the [Frontier &amp; CoWork](#frontier) guide.
