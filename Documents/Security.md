...
Microsoft Security Copilot has matured significantly through 2025 and into 2026. The most relevant changes for SOC analysts:

- **Deeper Defender XDR integration.** Security Copilot is now a first-class part of the unified Defender portal rather than a separate console — incident summarisation, guided response, and KQL generation appear inline next to the data they're acting on.
- **Embedded Intune query.** Natural-language device queries from inside Intune (e.g. "list all devices missing critical security updates") are generally available, simplifying compliance and device-security tasks.
- **Expanded plugin ecosystem.** More third-party plugins (Jamf, ServiceNow, Red Canary, Silverfort, Shodan, and a growing list) plus richer first-party integration with Entra, Purview, and Sentinel.
- **Stronger KQL generation.** Better handling of custom tables, multi-table joins, and time-window logic. Generated queries are increasingly the right answer the first time for common hunt patterns.
- **Script analysis with MITRE ATT&CK mapping.** Suspicious script analysis returns the techniques used, expected impact, and detection recommendations mapped to the framework.
- **Reporting and compliance.** Customisable executive summary templates and improved framework mapping (PCI, ISO, SOC 2, NIST) for compliance reporting.

For the live state of features and what's in preview, the **Microsoft 365 Roadmap** filtered by Security Copilot, and the **Microsoft Security blog**, remain the canonical sources.
