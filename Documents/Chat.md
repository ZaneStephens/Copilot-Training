# Microsoft 365 Copilot Chat: Comprehensive User Guide
...
Microsoft 365 Copilot Chat is the conversational interface to Copilot inside your work tenant. It lives in the Microsoft 365 Copilot app (web, Teams, desktop, and mobile), in Outlook, in Microsoft Edge when you're signed in with your work account, and as an embedded panel in many Microsoft 365 apps.

There are two licensing tiers of this same chat experience, and it's worth knowing which one you're using:

- **Free Microsoft 365 Copilot Chat** — included for everyone in the tenant with a qualifying Microsoft 365 work account, no paid Copilot licence required. Web grounding only (no access to your tenant data), but with full Enterprise Data Protection: prompts and responses stay inside the tenant boundary and are not used to train foundation models.
- **Paid Microsoft 365 Copilot** — adds the **Work** mode (grounded in your tenant data via Microsoft Graph), Notebooks, agents, reasoning agents (Researcher and Analyst), and integration inside Word, Excel, PowerPoint, Outlook, Teams, and the wider suite.

Both tiers use the same chat UI; the difference is what Copilot can see when you ask it a question. If your tenant only has the free tier deployed, the **Work** tab will be missing and Copilot will tell you it can't access organisational data.

The rest of this guide covers the full paid experience, with notes where free-tier behaviour differs.
...
Agents extend Copilot Chat with specialised capabilities. They show up in two places: a sidebar list of agents installed for you, and inline `@`-mentions inside the chat box.

**Microsoft-built agents.** The two most important are **Researcher** (deep, multi-step research with citations) and **Analyst** (data analysis with reproducible Python). Both are reasoning agents — they take longer to respond than a normal chat but produce structured, cited output. Available to tenants on the Frontier program first, then graduating to general availability. See the [Frontier &amp; CoWork](#frontier) guide for details.

**Tenant agents.** Built by your IT team in Copilot Studio, scoped to specific knowledge sources, connectors, or workflows. Examples: an HR policy agent grounded in the employee handbook, an "expenses" agent that knows the corporate policy and can draft expense queries.

**SharePoint agents.** Auto-created or hand-built against an individual SharePoint site, so you can chat with the contents of that site as if it were a knowledgeable colleague.

**Third-party and store agents.** Available through the Copilot agent store (subject to admin approval), covering common LOB systems and SaaS tools.

**How to invoke an agent:**
- Click it in the agent sidebar to start a chat scoped to that agent
- Use `@AgentName` in any chat to bring it into the current conversation
- Mention agents in a Notebook to keep them attached to a persistent workspace

Every agent runs with your existing permissions — it can only see what you can see — and your admin controls which agents are available to you.
